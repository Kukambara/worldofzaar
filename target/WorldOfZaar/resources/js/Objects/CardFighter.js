function CardFighter() {
	...
	this.Init = function (area,cardObject,cardName,cardSlogan,cardProperty) {
		...
		this.__proto__.background.SetSource(cardObject.subset.frontPath);
		...
		this.__proto__.background.area.width - this.borderSize * 3, this.borderSize), cardName);
	...
	this.InitSmall = function (cardObject,cardName) {
		...
		width, height), cardName);//new Area(new Point(0, 0), this.area.width, height ), "testName");
		width /= 2;
    ...
    this.SetDrawableArea = function(area){
        this.__proto__.SetDrawableArea(area);
    }
